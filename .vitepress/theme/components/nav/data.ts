import type { NavData } from './type'

export const NAV_DATA: NavData[] = [
    {
        title: '前端生态',
        items: [
            { icon: '', title: '阮一峰大佬的博客', desc: '科技爱好者周刊，每周五发布', link: 'https://www.ruanyifeng.com/blog/' },
            { icon: 'https://vueuse.org/favicon.svg', title: 'VueUse', desc: 'VueUse 是一个 Vue 3 的工具库，它提供了一些常用的工具函数，如：useMousePosition、useDebounceFn、useThrottleFn 等。', link: 'https://vueuse.org/' },
            { icon: 'https://3x.antdv.com/assets/logo.1ef800a8.svg', title: 'Ant Deaign Vue', desc: '公司两个项目组都用它', link: 'https://www.antdv.com/' },
            { icon: 'https://element-plus.org/images/element-plus-logo.svg', title: 'Element Plus', desc: '饿了么出品', link: 'https://element-plus.org/zh-CN/' },
            { icon: 'https://vitepress.dev/vitepress-logo-mini.svg', title: 'VitePress', desc: 'Vue官方团队维护的静态站点生成器', link: 'https://vitepress.dev/zh/' },
            { icon: 'https://www.radix-vue.com/logo.svg', title: 'Radix Vue', desc: 'Vue 无样式组件库', link: 'https://www.radix-vue.com/' },
            { icon: 'https://lhammer.cn/You-need-to-know-css/static/logo.png', title: 'You need to know css', desc: '常见CSS实现的动画效果，这里都有', link: 'https://lhammer.cn/You-need-to-know-css/' },
            { icon: 'https://uiverse.io/assets/logo-png-BXFOQKfc.png', title: 'UIVERSE', desc: '一些炫酷的UI小组件', link: 'http://uiverse.io' },
            { icon: 'https://uilibhub.com/favicon.png', title: 'UILibHub', desc: '精选最优质的 React、Vue UI、小程序组件库', link: 'https://uilibhub.com/zh' },
            { icon: 'https://loading.io/favicon.ico', title: 'loading.io', desc: 'Your SVG + GIF + PNG Ajax Loading Icons and Animation Generator', link: 'https://loading.io/' },
            { icon: 'https://primevue.org/favicon.ico', title: 'PrimeVue', desc: 'PrimeVue 是一个与设计无关的库，与其他UI库不同，它不强制使用某种样式', link: 'https://primevue.org' }
        ]
    },
    {
        title: 'AI',
        items: [
            { icon: '', title: 'ChatGPT', desc: 'AI 聊天机器人', link: 'https://chat.openai.com/' },
            { icon: '', title: 'DeepSeek', desc: '国产AI大模型', link: 'https://deepseek.cn/' },
            { icon: '', title: '豆包MarsCode', desc: '豆包旗下的编程助手，提供智能补全、智能预测、智能问答等能力，节省开发时间，释放脑海中的创造力', link: 'https://www.marscode.cn/home' },
        ]
    },
    {
        title: '实用工具',
        items: [
            { icon: 'https://www.json.cn/favicon.ico', title: 'Json.cn', desc: '在线json解析工具', link: 'https://www.json.cn/' },
            { icon: 'https://apifox.com/favicon.ico', title: 'APIfox接口调试工具', desc: '比Postman好用', link: 'https://app.apifox.com/main' },
            { icon: 'https://vuejsexamples.com/favicon.ico', title: 'vue实用案例收集', desc: 'vue实用案例收集：', link: 'https://vuejsexamples.com/' },
            { icon: 'https://logofa.st/favicon.ico', title: 'LogoFast | Make Beautiful Logos Fast and Free', desc: '在线生成logo', link: 'https://logofa.st' },
            { icon: 'https://svg.wtf/favicon.svg', title: '在线制作SVG图标', desc: '学习如何编写 SVG 代码的游乐场 - 包括一些有用的代码相关功能，如悬停查看属性的文档等。', link: 'https://svg.wtf/' },
            { icon: 'https://tool.lu/favicon.ico', title: '在线工具', desc: '在线工具箱', link: 'https://tool.lu/' },
            { icon: '', title: 'Pixabay', desc: '找素材图片的网站', link: 'https://pixabay.com' },
            { icon: '', title: 'Free SVG generators, color tools & web design tools', desc: '完全不用PS、AI就能用的背景图片工具！！！', link: 'https://www.fffuel.co/'  }
        ]
    },
    {
        title: '大数据',
        items: [
            { icon: '', title: 'Echarts', desc: 'Echarts官网', link: 'https://echarts.apache.org/zh/index.html' },
            { icon: '', title: 'Chartjs', desc: '轻量级，适用于现代 Web 的简单而灵活的 JavaScript 图表库', link: 'https://www.chartjs.org/' }
        ]
    },
    {
        title: 'web3D',
        items: [
            { icon: '', title: 'Three.js', desc: 'Three.js 是一个 JavaScript 3D 库，它使用 WebGL 来渲染 3D 图形。', link: 'https://threejs.org/' }
        ]
    },
    {
        title: '低代码',
        items: [
            { icon: '', title: 'Avuejs', desc: '基于Element的前端低代码组件库', link: '#' },
            { icon: '', title: 'Ideaz Element', desc: '基于element-plus，支持Vue3.x的中后台业务组件库.', link: '#' },
        ]
    },
    {
        title: '大神文章',
        items: [
            { icon: '', title: 'Easy Mock全解及使用_easymock使用方法-CSDN博客', desc: 'Easy Mock的使用', link: 'https://blog.csdn.net/w1418899532/article/details/91951603' },
            { icon: '', title: 'nginx配置反向代理 proxy_pass URL末尾加与不加/(斜杠)的区别_nginx代理后加url后半部分-CSDN博客', desc: 'Nginx配置', link: 'https://blog.csdn.net/willingtolove/article/details/108500198' },
            { icon: '', title: '前端朽木白大神秘籍', desc: '前端面试宝典', link: 'https://www.yuque.com/xiumubai/doc?mode=dark' },
            { icon: '', title: '页面循环请求优化方案', desc: '循环请求优化方案', link: 'https://blog.csdn.net/weixin_40476233/article/details/121292970' },
            { icon: '', title: 'Vue | 带有左右点击按钮可控制滑动的列表', desc: '', link: 'https://blog.csdn.net/weixin_44303404/article/details/123947427' },
            { icon: '', title: 'Vue3分段控制器（Segmented）', desc: '（分段控制器是antdesignvue4才有的新功能）', link: 'https://blog.csdn.net/Dandrose/article/details/140664033' },
            { icon: '', title: '实战：用 Vue3 实现一个 Message 消息组件在大多数 web 产品中，全局的 Message 组件占有较大的使 - 掘金', desc: 'Vue3 实现一个 Message 消息组件', link: 'https://juejin.cn/post/6975770170413776926' },
        ]
    },
    {
        title: '摸鱼必备',
        items: [
           { icon: '', title: 'Learn. Teach. Create.edclub', desc: '在线打字小游戏', link: 'https://www.edclub.com/' },
           { icon: '', title: 'H5魔塔', desc: '经典的魔塔游戏', link: 'https://h5mota.com/landing.php' },
           { icon: '', title: '格子纸打印', desc: '（纯属娱乐）有需要的时候可以用用', link: 'http://gridzzly.com/' },
           { icon: '', title: 'TypingClub', desc: '在线打字游戏', link: 'https://www.typingclub.com/' },
           { icon: '', title: 'TypingTest', desc: '在线打字游戏', link: 'https://www.typingtest.com/'  },
           { icon: '', title: '记忆力小游戏', desc: '记忆力小游戏', link: 'https://brain-trainer.pages.dev/' },

        ]
    }
]